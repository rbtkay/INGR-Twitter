...
class TweetHelper
{
	...
	public function setScoreForKeywords(User $user, KeywordRepository $k_repo, ScoreRepository $s_repo)
	{
		...
		foreach ($keywords as $keyword) {
			$keyword_in_tweets =
				$this->connection->get("search/tweets", ["q" => $keyword->getName(), "count" => "100"]);
	...
	public function setUserTweets(User $user, TweetRepository $t_repo)
	{
		...
		if (gettype($user_tweets)
			== "array") {                                         // if $tweets is an object it represents the error coming back from the twitter api.
			...
			$this->deleteOldTweets(
				[],
				$user,
				$t_repo
			); // when $tweets is an object (an error) we need to clear the table from the old tweets
		}
	}
